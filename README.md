...
Official TypeScript SDK for integrating [Levee](https://levee.sh) into your TypeScript/JavaScript applications.
...
```typescript
// Default configuration - connects to https://levee.sh/sdk/v1
...
const llm = new LLMClient("lv_your_api_key", {
  grpcAddress: "llm.levee.sh:9889", // optional, this is the default
});
...
console.log(
  `Contact: ${contact.name}, Status: ${contact.status}, Tags: ${contact.tags}`
);
console.log(
  `Emails sent: ${contact.emailsSent}, opened: ${contact.emailsOpened}, clicked: ${contact.emailsClicked}`
);
```
...
await levee.contacts.globalUnsubscribe(
  "user@example.com",
  "User requested to stop all emails"
);
```
...
console.log(
  `Email sent, message ID: ${result.messageId}, status: ${result.status}`
);
```
...
console.log(
  `Sent: ${status.sentAt}, Delivered: ${status.deliveredAt}, Opened: ${status.openedAt}`
);
...
const enrollments = await levee.sequences.getEnrollments(
  "user@example.com",
  "onboarding"
);
...
for (const e of enrollments) {
  console.log(
    `Sequence: ${e.sequenceName}, Step ${e.currentStep}/${e.totalSteps}, Status: ${e.status}`
  );
...
console.log(
  `Orders: ${customer.orderCount}, Subscriptions: ${customer.subscriptionCount}`
);
```
...
for (const inv of invoices) {
  console.log(
    `Invoice #${inv.number}: $${(inv.amountPaid / 100).toFixed(2)} ${
      inv.status
    }`
  );
...
for (const sub of subs) {
  console.log(
    `Subscription: ${sub.productName} - ${sub.status} ($${(
      sub.amountCents / 100
    ).toFixed(2)}/${sub.interval})`
  );
  console.log(
    `  Current period: ${sub.currentPeriodStart} to ${sub.currentPeriodEnd}`
  );
}
```
...
for (const p of payments) {
  console.log(
    `Payment: $${(p.amountCents / 100).toFixed(2)} ${p.status} via ${
      p.paymentMethod
    }`
  );
  if (p.receiptUrl) {
...
for (const price of product.prices ?? []) {
  console.log(
    `Price: $${(price.unitAmountCents / 100).toFixed(2)} ${price.currency}`
  );
  if (price.recurringInterval) {
    console.log(
      `  Recurring: every ${price.recurringIntervalCount} ${price.recurringInterval}`
    );
  }
}
...
console.log(
  "Time:",
  `${workshop.startTime} - ${workshop.endTime} ${workshop.timezone}`
);
console.log("Location:", workshop.locationType, workshop.locationDetails);
console.log(
  `Price: $${(workshop.priceCents / 100).toFixed(2)} ${workshop.currency}`
);
console.log(
  `Seats: ${workshop.seatsRemaining} of ${workshop.maxSeats} remaining`
);
...
console.log(
  "Secret:",
  result.secret,
  "(save this for signature verification!)"
);
```
...
const wh = await levee.webhooks.update(webhookId, {
  events: [WebhookEvents.PaymentSucceeded, WebhookEvents.PaymentFailed],
  active: true,
});
```
...
for (const log of logs) {
  console.log(
    `${log.deliveredAt}: ${log.event} (status ${log.statusCode}, ${log.durationMs}ms)`
  );
}
```
...
console.log(
  `Contacts: ${stats.totalContacts} total, ${stats.newContacts} new, ${stats.activeContacts} active`
);
console.log(
  `Emails: ${stats.emailsSent} sent, ${stats.openRate.toFixed(
    1
  )}% open rate, ${stats.clickRate.toFixed(1)}% click rate`
);
console.log(
  `Revenue: $${(stats.totalRevenue / 100).toFixed(2)} from ${
    stats.orderCount
  } orders`
);
```
...
console.log(
  `Totals: ${emailStats.totalSent} sent, ${emailStats.avgOpenRate.toFixed(
    1
  )}% open rate, ${emailStats.avgClickRate.toFixed(1)}% click rate`
);
...
console.log(
  `Orders: ${revenueStats.totalOrders}, Subscriptions: ${revenueStats.totalSubscriptions}, Churned: ${revenueStats.totalChurned}`
);
```
...
console.log(
  `Active: ${contactStats.totalActive}, Unsubscribed: ${contactStats.totalUnsubscribed}, Net growth: ${contactStats.netGrowth}`
);
```
